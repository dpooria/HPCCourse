...
PROGRAM Ex5
   USE m_Ex5_precision, ONLY: MK, MKS
   USE m_Ex5, ONLY: temp_new, temp_old, Nx, Ny, Nt, Dt, Dx, Dy, FL, D
   USE m_Ex5_init, ONLY: init_data
   USE m_Ex5_write, ONLY: write_to_file
   USE m_Ex5_copy, ONLY: copy_elemental!, copy_new_to_old
   USE m_Ex5_alloc, ONLY: alloc
   USE m_Ex5_diagnostics
   USE m_Ex5_parse_params, ONLY: read_input_params
   ...
   PRINT *, "Program Ex5 started at ", date, "-", time
   ...
   DEALLOCATE(temp_old)
   CALL DATE_AND_TIME(time=time, date=date)
   PRINT *, "Program Ex5 ended at ", date, "-" ,time
END PROGRAM Ex5
