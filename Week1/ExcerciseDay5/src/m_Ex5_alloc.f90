MODULE m_Ex5_alloc
   USE m_Ex5_precision, ONLY: MKS, MKD
   USE m_Ex5_write, ONLY: output_array_to_file
   IMPLICIT NONE
   INTERFACE alloc
...
CONTAINS
   ...
   SUBROUTINE allocd(a, N1, N2, info)
      ...
      IF (ALLOCATED(a)) THEN
         ...
         WRITE(fname, "(A,A,A)") "alloc_data_bkp/backup_from_m_Ex5_alloc_", TRIM(time), ".dat"
         CALL output_array_to_file(a, file_name=fname)
         DEALLOCATE(a)
      ENDIF
      ALLOCATE(a(N1, N2), stat=info)
