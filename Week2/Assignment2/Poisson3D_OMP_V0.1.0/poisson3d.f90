program poisson3d
   ...
   ! Read in namelist.
   call read_namelist()
   ! PRINT*, N, T0, algorithm, output, tolerance, itermax
   ...
   do j = 0, N+1
      ...
      enddo
   enddo
   !copy the same data to u_old
   u_old = u
   if (algorithm == 1) then
      call jacobi(u, time, iters, error, u_old, f, delta, N, tolerance, itermax)
   else
      print*, "Error! this version doesn't include the GaussSeidel method!"
      stop
   endif
...
contains
   ...
   subroutine read_namelist()
      ...
      namelist /INPUT/ N, itermax, T0, tolerance, output, algorithm, full
