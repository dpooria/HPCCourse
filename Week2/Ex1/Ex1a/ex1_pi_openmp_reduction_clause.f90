...
PROGRAM calculate_pi
   ...
   INTEGER :: system_tik, system_tok, count_rate, u, N0(6), i
   ...
   DO i = 1, SIZE(N0)
      ...
      WRITE(u, *) LOG10(REAL(N0(i), kind=MK)), e_time, 1.0/ (e_time + 1e-4)
   ENDDO
   CLOSE(u)
...
CONTAINS
   SUBROUTINE calc_pi(pi_, N0)
      ...
      !$OMP& shared(N, N_d) reduction(+: s)
      !$OMP do
      DO i = 1, N
         s = s + 4.0_MK / (1.0_MK + ((i - 0.5_MK)/ N_d) ** 2)
      ENDDO
      !$OMP end do
      !$OMP end parallel
      pi_ = s / N_d
